...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="(item, key, index) in bookstores.storeTypes">{{ key }} : {{ item }}</li>
      </ul>
      ...
      <ul>
        <li v-for="(item, key, index) in bookstores.openingHours">
          {{ key }} : {{ item.open }} to {{ item.close }}
        </li>
      </ul>
      ...
      <p>Our #1 seller:</p>
      <ul>
        <li v-for="seller in bookstores.topSellers">
          {{ seller }}
        </li>
      </ul>
    </section>
    ...
    <section class="lab-section">
      ...
      <h1>{{ orwell.name }}</h1>
      <p>BirthYear: {{ orwell.birthYear }}</p>
      
      <p>
        Achivement:
        <ul>
          <li v-for="work in orwell.famousWorks">
            {{ work.title }} in {{ work.year }}
          </li>
        </ul>
      </p>
      <img src="./assets/orwell.jpg" alt="Orwell Image">
    </section>
  </div>
</template>
